...
**排查案例:第一次跑通异步`retrieve`接口时踩的坑**

陈铭把`retrieve`函数从同步改成`async def`之后,做的第一件事是在本地写了一个最小化的调用脚本,验证接口能不能跑起来。第一次运行,终端直接甩出一堆`RuntimeWarning: coroutine 'KnowledgeBaseManager.retrieve' was never awaited`的警告,紧接着后面判断"检索结果是否为空"的那一行代码直接抛出`TypeError: object of type 'coroutine' has no len()`——他调用`retrieve`拿到的不是一个`list[RetrievedChunk]`,而是一个尚未被执行的协程对象本身。

陈铭对着这段报错愣了两三秒才反应过来:把一个函数从普通函数改成`async def`之后,所有调用这个函数的地方都必须补上`await`关键字,而他这次是"先改签名、后改调用处",偏偏漏改了验证脚本里的那一处。他后来跟赵子昂提起这个小插曲,还半开玩笑地说自己"被Python的语法糖坑了一下,写了这么多天异步代码,居然还能在这种基础地方栽跟头"。赵子昂倒是安慰他:"这种坑几乎每个写异步代码的人都踩过,IDE的类型检查有时候还看不出来,因为`coroutine`对象本身也是可以被赋值、被传递的,只有真正执行到用它的那一刻才会报错,排查起来比同步代码的低级错误更隐蔽。"

这次踩坑之后,陈铭给自己定了一条小规矩:凡是把一个函数从同步改成异步,第一件事不是急着去测试新逻辑对不对,而是先用IDE的"查找所有引用"功能,把这个函数的所有调用点全部列出来,一个个确认是否补上了`await`,确认无误之后再去验证业务逻辑本身,顺序不能颠倒——先改调用点、再测逻辑,比"改完就跑、出错再排查"要稳妥得多。这个习惯在后面几天的核心链路开发中,帮他省下了不少排查类似问题的时间。
...
把全局单例改成依赖注入之后,陈铭很快遇到了第二个坑。他在`deps.py`里定义`get_llm_client`这个依赖函数时,为了根据租户配置选择模型,需要引用`app.domain.tenant`模块里的`TenantContext`;而`app.domain.tenant`模块里的某个服务类,又反过来需要引用`deps.py`里定义的类型做类型注解,方便做静态类型检查。启动应用的时候,终端报出`ImportError: cannot import name 'get_llm_client' from partially initialized module 'app.api.deps' (most likely due to a circular import)`。

陈铭一开始的第一反应是把两个`import`语句的顺序换了一下试试,发现换了顺序错误只是从"A导入B失败"变成了"B导入A失败",本质问题没解决。他把这个情况在下午茶歇的时候跟老王简单描述了一下,老王只问了一句:"是不是`deps.py`里既有'构造依赖对象'的逻辑,又有'只是用来做类型注解'的类型引用?"陈铭想了想,确实如此。老王给的建议是:纯粹用于类型注解、不涉及实际运行时逻辑的引用,应该用`TYPE_CHECKING`这个标记包裹起来,配合字符串形式的类型注解(或者`from __future__ import annotations`),这样在运行时Python解释器根本不会真正执行这条import语句,只有静态类型检查工具(比如mypy或IDE的类型推断)才会去解析它,循环导入的问题自然就消失了,因为运行时的依赖关系图里根本没有那条边。陈铭按这个思路调整之后,应用顺利启动,他也把这条经验记进了骨架的编码规范文档里,提醒后面几天新增代码的同事注意区分"运行时依赖"和"仅用于类型注解的依赖"。
...
老王在Review的时候对这个设计给了正面评价,但也提了一个警示:"按业务方分路由前缀,好处是接口语义清晰,但坏处是如果以后业务方越来越多,每加一个业务方都要写一遍路由文件,容易产生大量重复代码。你现在这三个业务方的路由文件里,有多少是纯粹的样板代码?"陈铭当时没有完全想清楚这个问题,后来在写代码的过程中,提出了一个`RouterFactory`的模式——用一个工厂函数,根据传入的租户代码和该租户启用的功能列表,动态生成对应的路由集合,减少手写重复代码的比例,这部分在代码实战里会详细呈现。

值得一提的是,`RouterFactory`这个想法并不是陈铭一开始就规划好的,而是他在写第三个业务方(供应链)的路由文件时,发现自己几乎是把法务和人力的路由文件复制粘贴过来改了几个字段名,才后知后觉地意识到"这样下去不对"。他停下来跟老王简单沟通了一下这个念头,老王的反馈很直接:"发现自己在复制粘贴代码的那一刻,就是该停下来重新设计的信号,不要等复制粘贴了五六次才反应过来。"这句话陈铭后来专门记在了自己的个人笔记本上,作为一条工程习惯提醒。`RouterFactory`落地之后,三个业务方的路由注册代码从原本每个文件四五十行的重复样板,压缩成了每个业务方只需要提供一份"功能清单"配置(比如法务启用`contract_review`、`knowledge_qa`两项能力,供应链启用`knowledge_qa`、`inventory_alert`两项能力),再由工厂函数统一组装出FastAPI的`APIRouter`对象,新增一个功能点只需要在功能注册表里加一条映射,而不需要在三个业务方的路由文件里分别复制一遍相同的样板逻辑。
...
赵子昂当时在旁边听陈铭讲这段设计思路,提了一个补充问题:"如果`Chunk`的metadata字段是个自由格式的字典,不同分片器往里塞的字段名五花八门,后面写检索过滤逻辑的人怎么知道该用哪个字段名?"这个问题问得很实际,陈铭想了想,给出的方案是:`Chunk`的metadata分成"通用字段"和"扩展字段"两部分——通用字段(来源文档ID、分片位置、创建时间、敏感等级)在`BaseSplitter`基类里统一定义好字段名,任何具体分片器都不能随意改名;扩展字段则允许各个具体分片器自由定义,但要求每个分片器在自己的代码文件顶部用一段文档字符串,清楚地列出它往metadata里塞了哪些扩展字段、每个字段的含义和取值范围,方便后面写检索业务逻辑的同事查阅,而不需要去翻分片器的具体实现代码才能知道有哪些可用字段。老王后来在Review这段代码时,对这个"通用字段强约束+扩展字段自描述文档"的做法表示认可,认为这是在"完全统一"和"完全自由"这两个极端之间找到的一个务实的折中方案。
...
**排查案例:Alembic多Schema迁移的第一次失败**

11点半左右,陈铭开始写PostgreSQL的DDL脚本和Alembic迁移配置。他原本以为这一步会很顺利——毕竟建表语句本身并不复杂,无非是给三个租户schema各自建一套结构相同的表。但第一次执行`alembic upgrade head`的时候,终端直接报错:`sqlalchemy.exc.ProgrammingError: relation "tenant_legal.kb_documents" does not exist`。他查了一下,发现问题出在执行顺序上——他的迁移脚本里,创建schema的语句和创建表的语句是分别写在两个独立的迁移版本里的,但由于Alembic默认按照迁移文件的时间戳顺序执行,而他手动编辑迁移文件的时候不小心把"创建表"这个版本的时间戳改早了,导致Alembic先尝试建表,而对应的schema还没有被创建出来,自然找不到目标schema。

排查这个问题花了他将近二十分钟,一开始他怀疑是数据库连接权限的问题,还去检查了一遍数据库角色的权限配置,后来才想起来去看迁移版本的依赖关系图(`alembic history`命令可以打印出当前所有迁移版本的先后顺序)才发现是顺序错了。修复的方法很简单——把"创建schema"这个迁移版本的`down_revision`正确指向前一个版本,把"创建表"版本的`down_revision`指向"创建schema"版本,显式声明依赖关系,而不是依赖文件时间戳的隐式顺序。这次踩坑让陈铭意识到,Alembic的迁移版本链条一旦涉及到"手动调整顺序"的场景,一定要用`alembic history --verbose`显式核对一遍依赖关系,不能只靠文件名或者创建时间去猜测执行顺序,这个习惯他也记进了骨架文档的"迁移脚本编写注意事项"这一节里,提醒后面几天其他同事在新增迁移版本时避免踩同样的坑。
...
赵子昂现场挑了一份扫描件质量比较差的采购合同做测试,OCR识别出来的其中一段文本是这样的:"甲方应于合同签订后叉个工作日内支付预付款,金额为人民币壹佰万元整。"他一眼就看出"叉个"应该是"十个"被识别错了——原文里"十"字因为扫描时纸张有轻微褶皱,笔画被切断,OCR模型把它误判成了形近的"叉"字。这份文档的整体OCR置信度评分是0.79,低于0.85的阈值,按照设计好的规则,这一页会被自动标记为"待人工复核",不会直接进入向量化流程。老王看到这个真实案例后说:"这就是为什么我们坚持要设置置信度阈值,而不是无脑全量自动入库——'十个工作日'和'叉个工作日'在字面上看起来只差一个字,但如果这条错误信息真被存进知识库,后面业务人员问'预付款期限是多少天',系统给出的答案可能是完全无意义甚至误导性的内容。"周敏补充说,这类因扫描质量导致的识别错误,在寰宇集团历史合同里绝不是个例,她之前做人工审阅的时候就遇到过金额单位识别错误(比如"万元"被识别成"元",直接导致金额相差一万倍)的真实案例,这也是她坚持要求人工复核环节不能省略的原因之一。
...
赵子昂在处理那份600多条问答对的Excel FAQ列表时,发现了一个规整数据里也会出现的小麻烦:表格里有一列"最后更新人",里面填的是真实的HR工作人员姓名和工号,这一列本身对最终问答场景没有任何用处(用户不需要知道某条FAQ是谁更新的),但如果不做处理,这些真实姓名和工号就会跟着整行数据一起被转成文本、送进向量化流程,变成检索结果里可能被意外召回、展示给用户的内容。他把这个发现在下午的站立同步会上提了出来,老王当场给了一个原则性的判断:"不是所有列都要进知识库,拿到一份原始表格之后,第一步永远是先判断哪些列是'业务问答需要的信息',哪些列是'纯粹的管理元数据',管理元数据该丢的就要丢,不要因为'反正都在同一个文件里'就全部囫囵吞下去。"陈铭补充了一句:"这也提醒我们,知识库准备阶段最容易被忽视的风险,往往不是那些'看起来敏感'的字段,而是这种'看起来无害、但其实混进了不该暴露的信息'的边角字段,处理管道设计的时候要养成'先看schema、再决定字段取舍'的习惯,而不是全字段无差别处理。"
...
赵子昂在实际测试这个转换逻辑的时候,拿供应链管理部提供的"供应商资质清单.xlsx"练手,一运行就发现产出的语义描述全是乱码般的废话,类似"供应商名称为供应商资质清单,合作品类为2024年更新版,结算周期为nan"这种明显不对的句子。他打开原始Excel文件仔细看才发现,这份表格的第一行是合并单元格的大标题"供应商资质清单(2024年更新版)",第二行是空行,真正的表头(供应商名称、合作品类、结算周期等字段名)在第三行,数据从第四行才真正开始。他最初写的解析逻辑默认"第一行就是表头",完全没有考虑这种常见但不规范的Excel排版习惯,导致把标题行错当成表头、把空行和真表头都错当成数据行去处理。他后来在解析逻辑里加了一层启发式判断——如果某一行的非空单元格数量明显少于该表格的列数中位数,或者该行存在合并单元格样式,就跳过继续往下找,直到找到一行"看起来像正常表头"的行(非空单元格数量接近整表列数,且单元格内容以短文本、非数字为主)。
...
陈铭接着又抛出了一个自己一直在想的问题:"如果检索的时候,Milvus那边突然连不上了——比如网络抖动或者服务重启,`KnowledgeBaseManager.retrieve`要怎么处理?直接把异常甩给上层业务代码,还是在这一层做点什么?"这个问题引发了三人一小段讨论。赵子昂的直觉是"应该重试几次",老王则从更全局的角度补充:"重试是对的,但重试次数和间隔要有讲究,不能无限重试导致请求一直挂着不返回,也要考虑'重试打满之后怎么给用户一个体面的降级提示',而不是让用户看到一个冷冰冰的500错误。"三人最终商定的方案是:在基础设施层的Milvus客户端封装里加入指数退避的重试机制(初次失败后等待0.5秒重试,失败则等待1秒,最多重试3次),如果3次都失败,向上抛出一个统一的`KnowledgeBaseUnavailableError`异常,由接入层统一捕获,返回一个友好的错误提示,而不是把底层Milvus SDK的原始异常堆栈直接暴露给调用方——这也是为了保证不管底层用的是Milvus还是未来换成别的向量库,领域层和接入层看到的异常类型都是统一的、经过封装的业务异常,不会因为更换底层实现而导致上层异常处理代码也要跟着改。

老王在这段讨论快结束的时候,半开玩笑地对陈铭和赵子昂说:"你们俩现在讨论问题的方式,已经不太像是'写代码遇到问题就去查'的思路了,更像是在'预判所有可能出问题的场景,提前把兜底方案设计好'。这个转变,是从'实习生思维'到'工程师思维'很关键的一步。"这句话让陈铭想起了几十天前刚接触"苍穹企业级智能体中台"项目时,自己写代码基本是"先跑起来再说,出了bug再改"的路子,而今天讨论`KnowledgeBaseManager`接口设计的这四十分钟,几乎每一个决策点,都是先设想清楚"如果这里出问题会怎样",再决定该怎么设计。
...
林薇虽然主要负责前端,但今天下午也旁听了大半个小时的知识库讨论,她提了一个从产品体验角度出发的问题:"如果某份文档解析失败,或者OCR置信度不够被打回人工复核,前端要不要有个地方能看到这些'处理失败/待复核'的清单?不然客户那边的业务人员根本不知道自己上传的合同为什么问答不出结果。"这个问题一下子点出了一个团队之前没有认真讨论过的空白点——今天设计的管道里确实有"待复核"这个状态,但只停留在后端日志和数据库字段层面,还没有任何界面能让业务人员感知到。老王认可了这个问题的价值,当场决定把"知识库处理状态看板"这个小功能加进了后续几天的待办事项里,虽然不在今天的DoD范围内,但记录下来避免遗漏,并让林薇先在原型稿里预留出对应的界面位置,方便后续接入。
...
复盘会议接近尾声时,周敏通过视频连线补充了一条合规角度的收尾意见:"今天从我这边看到的骨架设计,把'Schema级隔离'和'敏感条款分级标签'这两块硬性要求都落地了,这一点我是满意的。但我要提前说一句,接下来几天,一旦涉及到具体业务接口开发,每一个新增的对外接口,我都需要过一遍它涉及的数据字段,判断是否需要在响应体里做脱敏处理,这个review环节请大家提前预留时间,不要等接口写完了、联调完了,才发现某个字段不该原样返回,那时候改动成本就高了。"老王当场答应了这个要求,并把"接口设计阶段同步拉周敏做数据字段审查"写进了后续几天的流程规范里,要求林薇和陈铭在设计新接口的Pydantic响应模型时,主动同步给周敏过一遍,而不是被动等待事后审查。

老王还顺带统计了一下今天的产出规模:骨架代码加上三个知识库的初始化脚本,一共提交了十七次commit,新增代码接近两千四百行(不含测试代码和迁移脚本),其中一多半的时间花在租户上下文、数据隔离、知识库抽象层这几块"看不见"的基础设施上,直接对外可见的业务接口反而只占了很小的比例。老王对此评价说:"这个比例在骨架搭建阶段是正常的,甚至是健康的——如果哪天我们发现骨架阶段大部分时间都花在写业务接口上,那反而要警惕,是不是把'先搭好地基'和'先出活儿'的优先级搞反了。"
...
今天的作业围绕着骨架搭建和知识库准备这两条主线,重点考察大家能不能把今天讨论过的架构决策"举一反三"——不是死记硬背今天代码里具体用了哪个类名、哪个字段名,而是要真正理解每一个设计决策背后"为什么这样做、不这样做会有什么后果"这一层逻辑。建议大家在做题之前,先把今天课堂笔记里几个排查案例(比如异步接口漏加`await`、Alembic迁移顺序错乱、Excel表头解析错位)重新过一遍,这些真实踩坑的细节往往比抽象的架构原则更能帮助理解设计决策的必要性。
...
7.(选做,综合思考题)今天下午,林薇提出了"知识库处理状态看板"这个需求,团队当时的决定是"记录下来,不在今天落地"。请结合今天晨会上老王反复强调的"骨架搭建阶段要抓大放小、聚焦当天DoD"的原则,思考并简述:一个团队在封闭开发冲刺这种高压排期下,应该用什么样的标准去判断一个"合理但不紧急"的新需求,应该"当场加进今天的任务"还是"记录后延后处理"?请结合今天寰宇项目的具体场景,至少给出两条可操作的判断标准。
...
判断一个"合理但不紧急"的新需求在封闭开发冲刺期间应该"当场加入今天任务"还是"记录后延后处理",可以参考以下几条可操作的标准。

第一条标准是"是否影响当天DoD涉及的核心链路"。今天的DoD明确要求的是骨架搭建完成、多租户数据模型可用、三个知识库处理管道跑通,林薇提出的"知识库处理状态看板"是一个前端展示层面的增强功能,不属于任何一条DoD的必要条件,即使不做,也不影响今天验收标准的达成,因此可以判断为"不紧急"。反过来说,如果今天讨论中发现某个遗漏点直接关系到DoD能不能达成(比如发现多租户隔离设计存在漏洞),那就必须当场处理,不能拖延。

第二条标准是"延后处理的成本是否会随时间推移而显著增加"。像"知识库处理状态看板"这类纯粹的前端展示功能,今天不做,明天、后天再做,只要底层的状态字段(比如"待复核""处理失败")已经在数据模型里预留好了,后续接入的成本基本是固定的,不会因为拖延而显著增加。但如果是类似"数据隔离机制"这种一旦定型就很难推翻重做的底层架构决策,拖到后面几天再处理,返工成本会随着上层代码越写越多而急剧上升,这类需求即使当下看起来不紧急,也应该尽早处理掉。

第三条标准是"是否已经有现成的数据结构或接口支撑,只是暂时缺一个呈现的界面"。今天团队之所以能放心地把这个需求往后推,一个重要前提是老王确认了"待复核"这个状态已经作为字段存在于数据库里,只是还没有一个前端界面把它暴露出来——地基已经打好,缺的只是"最后一步的呈现",这种情况下延后处理的风险是可控的。如果反过来,某个需求连底层数据结构都还不存在,一旦拖到后面才发现需要为它专门改动数据模型,就可能引发连锁的返工,这种情况就不适合简单地"记录后延后",需要更谨慎地评估。

综合以上几条标准,可以得出一个通用的原则:面对高压排期下的新增需求,团队不应该简单地用"紧急"或"重要"这种笼统的主观判断来决定优先级,而应该具体分析这个需求跟当前DoD的关联程度、拖延处理的边际成本变化趋势,以及底层支撑是否已经就位,这样才能既不因为"今天必须全干完"的完美主义而拖慢冲刺节奏,也不因为"先放着以后再说"的惰性而埋下后期返工的隐患。
