...
陈铭紧接着追问了一个类似性质的问题:"AC-4.3里写的是'当Rerank后最高相关性分数低于预设阈值时,系统返回明确提示',这个'预设阈值'具体应该定成多少,PRD里并没有给出一个数字。"这个问题让会议室安静了几秒钟——显然这也是一个此前没有被彻底讨论过的细节。老王想了一下,给出的思路是:"阈值这类数字,不应该在没有真实数据支撑的情况下拍脑袋定出来,应该先跑一批Day33积累下来的评估样本,把每一条'知识库里确实没有相关内容'的问题对应的Rerank最高分记录下来,再把每一条'知识库里确实有相关内容'的问题对应的Rerank最高分也记录下来,画出两组分数的分布,找一个能尽量把两组分开的分界点,这个分界点就是阈值的初始值。"赵磊在旁边补充说,这个阈值不会是一次定死的,验收阶段他会专门测试几个"知识库里确实没有覆盖"的边界问题,如果发现系统给出了看似合理却缺乏依据的答案,或者反过来把明明能查到的内容误判为"找不到",都会作为缺陷提出来,要求团队回头调整这个阈值。陈铭把这段对话理解为——很多PRD里看似简单的一句话,背后对应的往往是一套需要用真实数据校准的工程判断,而不是凭直觉写一个数字就能了事,这也是他今天对"验收标准"这四个字有了更具体认识的又一处细节。林悦补了一句总结,说这种"先拿数据校准参数,而不是拍脑袋写死"的做法,几乎贯穿了这份PRD里所有涉及具体数值的条款——无论是混合检索候选集的Top-20,还是多轮追问的"最近3轮",背后都对应着或多或少的实际依据,只是有些依据是历史项目经验,有些依据是当场决定要补做的一次小实验,这也是产品经理和技术负责人配合定需求时,需要反复来回打磨的地方。
...
老王在讲完分层原则之后,又补了一段关于日志和可观测性的要求,这段内容原本不在架构图上,却被他认为同样重要:"分层做得再干净,如果系统跑起来之后出了问题却查不出是哪一层出的问题,分层的价值也会打折扣。所以我们从今天开始,要求每一层关键的服务方法,进出口都要打日志——进来的时候记录关键参数(比如知识库ID、请求的问题文本前几十个字符),出去的时候记录耗时和结果摘要(比如检索到了几条候选、Rerank打分区间)。"陈铭当场问,这样是不是会让日志量变得很大,老王的回答是:"封闭冲刺期间日志量大一点没关系,我们不是要节省磁盘,是要保证联调和验收阶段一旦出问题,能在日志里迅速定位到底是检索层召回不到内容,还是生成层没把引用拼对,还是数据库层查询条件漏了租户过滤。这几层任何一层出问题,表现出来的症状可能是同一个——回答不对或者报错,但排查思路完全不同,日志的粒度决定了排查效率。"这段话让陈铭想起自己刚入职时写脚本从来不打日志、出了问题就靠反复加`print`硬调的笨办法,现在再回头看,能体会到"可观测性"和"业务逻辑本身"其实是同等重要的两件事,只是后者更容易被优先关注,前者往往要吃过排查的苦头才会真正重视。老王还顺带提了一句要求——所有日志里涉及租户和用户标识的部分必须原样记录,不能因为担心日志文件太乱而省略,因为一旦真的出现跨租户串号这类红线问题,日志里如果没有`kb_id`和`user_id`,排查起来会比问题本身更麻烦。
...
下午开发正式启动前,老王额外花了十分钟,专门讲了一遍`deps.py`这个文件的设计用意——这是陈铭过去写命令行脚本时完全没有遇到过的一类文件。老王解释:"FastAPI里的依赖注入机制,可以让我们把'获取数据库会话''校验知识库是否存在''解析当前请求归属的租户'这类每个接口都要重复做的事情,统一写成一个个可复用的函数,通过`Depends()`挂在每个路由函数的参数上,而不是在每一个接口函数体的第一行都手写一遍相同的校验代码。"他现场写了一个简化示例,展示`get_db()`如何被多个路由复用,又展示了一个`verify_kb_exists(kb_id)`依赖如何在文档上传、问答、会话创建三个不同的接口里被同样引用,却只需要维护一份实现。陈铭当时对这个机制还有点将信将疑,直到下午真正写`chat.py`路由的时候才切身体会到好处——原本担心的"每个接口都要重复写租户校验逻辑,万一哪个接口漏写了就是隔离漏洞"这个隐患,通过依赖注入变成了"只要接口签名里带上了这个依赖,校验逻辑就一定会被执行",这跟上午会话管理讨论时"把正确用法做成类型层面强制要求"的思路是同一种工程哲学的不同体现,陈铭在笔记本上把这两处联系了起来,标注了一句"依赖注入=用机制代替自觉"。张凡在旁边补充说,这个思路其实和他们之前在苍穹平台其他模块见过的中间件设计是相通的——凡是"每个接口都得做、又不能漏做"的事情,优先考虑能不能用框架机制强制保证,而不是依赖开发者的自觉性,这是团队这段时间反复被灌输的一条工程原则。
...
晚自习联调过程中,陈铭还额外补充了一处此前没有被专门讨论过的边界情况——批量上传的文件里,如果同时包含几份内容几乎完全重复的文档(比如同一份操作手册的不同版本被重复上传),系统目前的行为是把它们当成完全独立的文档分别处理,检索结果里可能会出现好几个几乎一样的片段挤占Top-K的位置,挤掉了其他真正有价值的内容。他把这个现象记录下来,发给老王,老王的回复是:"这是一个真实存在的问题,但今天不是解决它的时候——去重涉及到内容相似度比较,是需要额外设计的一套逻辑,不属于今天五大功能点里明确要求的验收范围,先记到技术债务清单里,不要现在就动手改,不然容易在冲刺期临时引入一个新的、没有充分测试过的模块,反而增加风险。"陈铭听完觉得有道理,把这一条也补进了张凡下午提议维护的"已知技术债务"清单里,清单当天结束前已经积累了三条:文档处理异步化、内容重复去重、以及用户体系尚未真正落地。他还顺手在清单每一条后面补了一句简短的处置建议,方便后续迭代时不用再重新讨论一遍背景,比如"内容重复去重"这一条,他写的备注是"建议后续接入基于embedding相似度的去重步骤,阈值需要结合真实文档样本重新评估,不建议直接沿用检索阶段的相似度阈值"。
