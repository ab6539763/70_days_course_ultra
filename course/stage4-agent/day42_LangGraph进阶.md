...
林悦看到这张图之后,专门在群里回了一句话,说这个"存档—读档"的比喻,她打算原样讲给祺瑞集团的信息化负责人听,因为对方之前一直担心"AI这种东西是不是不太稳定,会不会把审批单据弄丢",用游戏存档这个大家都熟悉的场景做类比,比讲"持久化检查点""状态快照"这些技术词汇更容易让业务方建立信任感。王振宇听完说这就是好的技术类比该有的样子——不是把复杂的东西讲得更复杂来显示专业,而是能让完全不懂技术的人,凭直觉就明白"这件事是可靠的"。
...
为了让陈铭把这三种选型的取舍记得更牢,王振宇在白板上又画了一张对照表,陈铭抄在了笔记本上:

| 维度 | MemorySaver | SqliteSaver | PostgresSaver |
| --- | --- | --- | --- |
| 存储介质 | 进程内存字典 | 本地SQLite文件 | 独立部署的PostgreSQL |
| 进程重启后是否存活 | 否,清零 | 是,文件还在 | 是,数据库还在 |
| 是否支持多实例并发访问同一份数据 | 不适用(数据本身就是私有的) | 有限支持,并发写入容易冲突 | 支持,天然为并发设计 |
| 适用规模 | 本地调试、单元测试 | 单机部署、课堂演示、小规模验证 | 生产环境、集群部署 |
| 是否需要额外运维 | 不需要 | 几乎不需要,文件即数据 | 需要,但可以复用现有数据库运维体系 |
| 祺瑞场景是否可用 | 绝对不可用 | 仅可用于验证阶段 | 目标生产方案 |

王振宇让陈铭盯着"进程重启后是否存活"这一行多看两秒,他说这行的答案基本上决定了前面五行讨论的一切都有没有意义——如果这一行是"否",后面聊性能、聊并发、聊运维成本都是空谈,因为客户根本不可能接受一个连"重启不丢数据"都做不到的方案。

陈铭当时还顺手在本地敲了一个"反面例子"验证王振宇的说法。他先用`MemorySaver`编译了一个带`interrupt_before`的小图,`invoke`跑到中断点,然后在同一个Python进程里,故意重新创建了一个新的图对象(模拟"服务被重启,重新构建了一份运行时"),再拿着同一个`thread_id`去调用`get_state`,结果拿到的`values`是空的,`next`也是空元组,状态查询不到任何"正在等待审批"的痕迹,就好像这个流程实例从来没有存在过。换成`SqliteSaver`之后重复同样的操作,新建的图对象依然能通过`thread_id`查出完整的状态和`next`字段。这个对比实验让陈铭对"没有持久化的中断只是假的中断"这句话彻底没有了疑问——他说这比听十遍道理更管用,因为亲眼看到数据"啪"地一下就没了。
...
陈铭上午确实踩过一次这个坑。他图省事,写了一个没带`configurable`的`config = {}`就直接拿去`invoke`,程序表面上跑得很顺畅,大纲生成、质量校验都正常执行,他还以为一切正常,直到写到"模拟重启后恢复"这一步,调用`get_state({})`,发现返回的状态快照是空的,他愣了几秒,回头去翻官方文档,才发现自己压根没有传`thread_id`,严格来说这次调用相当于每次都在用一个隐式的、不受控制的默认线程标识在跑,而他后续用来做"重启恢复"验证的那个`config`,和第一次`invoke`用的那个`config`在对象上是两个不同的空字典实例,虽然内容看起来"一样"(都是空的),但如果具体版本里默认线程标识的生成逻辑与调用上下文有关,就完全可能对应不到同一个thread。他把这件事形容成"我以为我在给同一个人写信,结果信封上什么都没写,连自己都不知道信寄到哪儿去了"。王振宇听完笑了一下,说这是几乎所有第一次接触Checkpointer的人都会踩的坑,他的建议是:**永远显式声明`thread_id`,并且把它的生成规则写成团队共识的一部分**——比如统一约定"业务单据编号即thread_id",而不要依赖任何隐式的默认行为,哪怕当前版本的默认行为看起来"凑巧能用",换一个版本、换一个部署环境,这种隐式依赖随时可能失效。
...
陈铭当时追问了一句:`update_state`改的这些字段,会不会因为下一个要执行的节点又重新计算了一遍,把人工改的值给覆盖回去?王振宇说这是一个很好的问题,答案是"要看节点怎么写"——`update_state`本身只是往Checkpointer里追加了一个新的检查点,记录了"这个字段被人工改成了什么",但它不会改变图接下来的执行逻辑;如果紧接着要执行的节点,恰好会重新计算并覆盖同一个字段(比如某个节点每次都无条件地从原始单据数据重新算一遍金额,而不是"看状态里有没有已经存在的值再决定要不要算"),那人工改的值确实会被冲掉。他让陈铭记住一条设计原则:**允许人工介入修改的字段,后续节点的写法应该是"尊重已有值,除非有明确理由要重新计算",而不是每次都无条件地从头算一遍**,否则`update_state`这个能力在实际业务里会变成一个"看起来能用,但用了也白用"的摆设。

王振宇还提到一个`get_state`返回结果里容易被忽略的字段——`created_at`和`metadata`。`created_at`记录了这个检查点写入的时间,在祺瑞的场景里,业务系统可以拿这个时间戳去计算"这张单子已经在当前节点等了多久",用来驱动催办逻辑(比如超过24小时没有响应就自动发一条提醒);`metadata`里可以携带一些额外的调试信息,比如是哪一次`invoke`调用触发了这个检查点、当时传入的输入是什么摘要,这在生产环境排查"这张单子为什么卡在这里"的问题时非常有用,比单纯看业务日志更直接,因为它是引擎自己记录下来的、和状态变化强绑定的元数据。
...
上午练习到中途,陈铭的终端里冒出过几次实实在在的报错,王振宇没有直接告诉他答案,而是让他自己盯着报错信息一步一步查,课件里把这几次排查过程完整记下来,方便后来人对照。

第一次报错发生在陈铭第一次尝试恢复一个中断的流程时,他当时直接调用了`graph.invoke({"approved": True}, config=config)`,而不是用`Command(resume=...)`包一层,程序没有直接崩溃,但表现出来的行为很反常——图并没有从中断点继续往下走,而是把`{"approved": True}`当成了一份全新的普通输入,重新从`START`节点开始跑了一遍,导致大纲又被重新生成了一次。陈铭盯着日志看了半天才意识到问题:`invoke`的第一个参数如果不是`Command`对象,LangGraph没有办法知道"这是要恢复一个挂起的中断",它只会把这个参数当成常规的状态输入。王振宇总结说,这是一个"不报错但行为完全不对"的典型陷阱,比直接抛异常更难发现,因为程序会"顺利"地跑完,只是跑出了一个完全错误的结果。

第二次报错更直接,陈铭在另一次尝试中故意漏传了`config`参数,控制台里跳出了类似下面这样的异常信息(为了课件的可读性,这里保留了报错的关键结构,细节措辞可能随版本略有差异):

```text
ValueError: No checkpointer set for this StateGraph. Cannot save or load state without a configured checkpointer and thread_id in config.
```

这类报错反而是"好报错"——王振宇说,系统宁可直接崩溃告诉你"配置不对",也好过静默地跑出一个错误结果。陈铭对照代码检查了一遍,发现是他在演示脚本里手写了一个新的`config = {}`,而不是复用`new_thread_config`这个统一封装的工具函数,随手写的代码里漏掉了`configurable`那一层嵌套。这件事之后,团队内部约定,所有涉及`thread_id`的地方,必须统一通过`new_thread_config`这样的工具函数生成`config`,不允许在业务代码里手写字典字面量,减少因为拼写或结构疏漏导致的类似问题。

第三次是一个更隐蔽的问题:陈铭在写`outline_human_review`节点时,一开始把`decision = interrupt(review_payload)`这一行放在了一个`try/except`块里,`except`分支里写了一段"兜底逻辑",本意是想在中断相关API出问题时有个降级处理。结果他发现,只要一恢复执行,`except`分支就会被触发,大纲审核环节根本走不到正常的判断逻辑。王振宇看了代码后解释,LangGraph的`interrupt()`底层是通过抛出一个特殊的异常(`GraphInterrupt`及相关机制)来实现"挂起"效果的,如果业务代码用一个宽泛的`except Exception`把这个内部机制用的异常吞掉了,就会打断引擎正常的挂起流程,表现出来的症状五花八门,取决于具体怎么写的兜底逻辑。这条经验被王振宇提炼成一句话记在了白板边上:"**永远不要在可能调用`interrupt()`的代码路径上,写宽泛的`except Exception`,除非你非常清楚自己在干什么。**"这也是陈铭当天笔记里专门用红笔标出来的一条。
...
这个对照表还引出了一个更细节的问题:陈铭问,既然独立节点方案里"每一级审批"都是单独一个节点,那`thread_id`要不要跟着换?王振宇说不需要,也绝对不应该换——`thread_id`标识的是"这一整张单据的生命周期",不管它内部走了多少个节点、经历了多少级审批,从提交到最终归档,都应该始终绑定同一个`thread_id`,这样`get_state_history`才能拿到这张单据完整的、连续的历史轨迹;如果每一级审批都换一个新的`thread_id`,相当于人为地把一条本该连续的业务生命周期切成了好几段互不相关的记录,业务人员想查"这张单子从提交到现在经历了什么",就要跨好几个`thread_id`去拼接,平白增加了不必要的复杂度。这条提醒被陈铭记在了"设计原则"这一页,和上午那条"永远显式声明`thread_id`"的提醒放在了一起,他说这两条加起来,基本上把`thread_id`该怎么用、不该怎么用讲全了。

王振宇最后补充了一个上午没细讲、但和这里的话题强相关的点:如果同一张单据(同一个`thread_id`)在系统里被同时打开了两个不同的操作界面(比如客户的两名员工都点开了同一张单子准备操作),会不会出现并发问题?他说这属于"同一个thread_id的并发写入"场景,LangGraph本身的Checkpointer机制保证的是"每一次写入都是一个完整的、带版本信息的检查点",不会出现数据损坏或者写入过程中数据错乱,但如果两个人几乎同时提交了不同的审批意见,后提交的那一次操作对应的检查点会成为"最新"的状态,先提交的那次操作的意图有可能被覆盖——这在技术上不算错误,但在业务上属于"并发冲突"需要额外处理的场景,通常的做法是业务层面在提交操作前先做一次"当前状态是否仍处于等待该角色操作"的校验,如果状态已经变化(说明已经有人先操作了),就提示后来的操作者"该单据已被处理,请刷新查看最新状态",而不是让两次操作都不明不白地生效。王振宇说这条内容严格来说超出了今天的教学范围,但既然陈铭问到了并发场景,还是值得提一句,免得后面真遇到类似问题时完全没有方向。
...
陈铭追问了一个很实际的问题:子图内部要不要单独配一个Checkpointer?王振宇的回答是分两种情况讨论。如果子图的生命周期完全依附于父图的一次节点执行——也就是说,子图从进入到跑完,是在父图的同一个超步里一次性完成的,中间不需要单独挂起等待人工——那么子图不需要也不应该配自己的Checkpointer,它的状态天然就是父图那一次节点调用的一个"内部实现细节",父图的持久化机制已经足够覆盖它,晚上写作Agent里的"章节自审子图"就是这一类,自审内部的"审查—修订"循环全部在一次`chapter_review_node`调用里跑完,不需要跨越多次`invoke`。但如果子图本身也需要独立地挂起等待外部事件(比如子图内部也有一个需要人工审批的节点,而这个审批的等待时间可能跨越很长时间),那子图就需要有自己独立的、可寻址的执行生命周期,这种情况下,更合适的做法往往不是简单地把子图当成父图的一个内嵌节点,而是考虑把这部分逻辑改造成一个独立的、可以单独被`invoke`和持久化的图,通过业务编排层(而不是LangGraph内部的父子图机制)去协调两个独立图之间的调用关系。王振宇说这条边界要划清楚,不然容易把"子图"这个偏静态的复用工具,用出"跨进程长时间独立挂起"这种它本不擅长解决的效果,遇到问题也不好排查。

课后陈铭还专门做了个小实验,验证子图状态schema不一致时会发生什么。他故意在子图`ChapterReviewState`里保留了一个`chapter_index`字段,但在包装函数`chapter_review_node`传给子图的`sub_input`里漏写了这个字段,子图跑起来之后,`state.get("chapter_index")`拿到的是`None`,由于当天的子图逻辑没有直接用到这个字段做关键判断,程序没有报错,只是这个字段全程都是空的,直到陈铭在打印调试信息时才发现不对。他把这个教训总结成一句话:"子图的输入不会因为字段名对得上就自动填充,该传的都要显式传,漏了不会报错,只会悄悄变成`None`,这种坑比直接报错更烦人。"王振宇听完补充说,这也是为什么很多团队会在子图入口处专门写一层校验函数,对必需字段做显式检查,宁可在子图刚开始执行时就快速失败抛出清晰的报错,也不要让一个空值悄无声息地流转到后面某个环节才暴露问题。
...
陈铭当天下午其实亲身经历了这个"命门"级别的坑,而且是踩在晚上写代码之前的一次课堂小练习里。王振宇让大家现场写一个三分支并行求和的小demo,陈铭图快,把状态字段直接定义成了普通的`list[int]`,没有加`Annotated`,也没有配reducer,三个并行分支各自返回`{"results": [state["value"] * 2]}`。跑起来之后,他打印`final_state["results"]`,发现列表里只有一个元素,而不是预期的三个。他一开始怀疑是`Send`没有真正并行执行,还专门加了`print`语句去看三个分支是不是真的都跑了——确认三个分支都执行了,`print`能看到三条不同的输出,但最终汇总到父状态里的`results`字段却只剩一条。他把这个现象截图发到了小组群里问,王振宇没有直接给答案,只反问了一句:"你的`results`字段,状态定义里写的是什么类型?有没有告诉引擎,遇到多个分支同时往这个字段写数据的时候,应该怎么处理?"陈铭这才回去翻状态定义,意识到默认情况下(没有显式reducer时),后写入的值会直接覆盖先写入的值,三个并行分支几乎同时执行完毕,谁的写入"生效"取决于合并顺序,表现出来就是"结果只剩一条,而且具体剩哪一条还不一定"。把字段改成`Annotated[list[int], operator.add]`之后,三条结果就都保留下来了。他说这个练习让他真正理解了"reducer不是可选的语法糖,是并行结果能不能保留下来的开关"。

这里王振宇顺带把reducer的概念往前做了一次呼应——他提到,其实早在更早的课程里讲状态设计基础的时候,就已经埋下过"状态更新是靠reducer函数决定如何合并新旧值"这个概念,只是那时候大多数场景都是"最新值覆盖旧值"这种最简单的reducer,不太容易感受到reducer本身的存在感;直到今天遇到并行节点这种"多个分支同时往同一个字段写数据"的场景,reducer的作用才第一次变得"图不设计好就会直接出问题"这么显眼。他说这是一个很好的例子,说明很多技术概念在刚学的时候可能觉得"平平无奇,好像随便选个默认行为都没差",但随着场景变复杂,当初打的基础会在意想不到的地方决定成败。
...
为了让"退避倍数"这个抽象概念更直观,王振宇让陈铭现场把`RetryPolicy(max_attempts=4, initial_interval=0.5, backoff_factor=2.0, max_interval=30.0)`这组参数的实际等待时间手算了一遍,画成一张时间轴表格贴在了笔记本上:

| 尝试次数 | 本次尝试前的等待时长 | 说明 |
| --- | --- | --- |
| 第1次尝试 | 0秒 | 首次执行,不等待 |
| 第2次尝试(第1次重试) | 0.5秒 | 等于initial_interval |
| 第3次尝试(第2次重试) | 1.0秒 | 0.5 × backoff_factor(2.0) |
| 第4次尝试(第3次重试) | 2.0秒 | 1.0 × backoff_factor(2.0) |
| 若还失败 | 不再重试,直接抛出异常 | 已达到max_attempts=4的上限 |

陈铭算完之后感叹了一句:"这么一算,从第一次失败到最终放弃,总共也就多等了三点五秒左右,比人工发现问题再手动重试快得多。"王振宇点头,说这正是重试策略的价值所在——它解决的不是"让系统永远不失败",而是"用极低的、用户几乎感知不到的额外等待成本,去吸收掉那些本来会造成明显故障的瞬时性抖动"。他又补了一句,如果把`backoff_factor`设得过大或者`max_interval`设得过高,单次节点执行的最长耗时可能被拉得很长,对于一些用户在前台同步等待结果的场景(比如客户在界面上点了"提交"之后停在那里等结果),这种设计反而会带来不好的体验,所以`max_interval`这个"封顶值"不能随手设一个很大的数字,要结合具体节点在业务里对延迟的容忍度来定,这也是为什么写作Agent的代码里,专门为`polish_node`这种"用户明确在等待"的节点配置了参数更保守的`strict_retry_policy`,而不是和其他节点共用同一套默认策略。

王振宇还提到一个容易被忽略的细节:`jitter`参数。如果多个流程实例几乎在同一时刻遇到同一个第三方接口的短暂故障(比如夜里那次发票查验接口抖动,可能同时影响了几十张正在处理的报销单),如果所有失败的重试都严格按照同样的退避时间点重新发起请求,就会形成"重试请求集中打过去"的二次冲击,反而可能让刚刚开始恢复的第三方服务再次过载。加入`jitter`(在计算出的等待时间上叠加一个小的随机抖动)之后,各个流程实例的重试时间点会被自然地错开,避免这种"重试风暴"的连锁反应。这个细节在课本里往往一句话带过,但王振宇说这是他自己在以前的项目里真吃过亏才总结出来的经验,值得多说两句。
...
陈铭把这句话记下来之后,顺手在下面补了一行自己的理解:"子图、并行、重试这三件事,分别对应软件工程里三个很老的话题——代码复用、性能优化、容错设计,只是今天是用LangGraph这套图编排的语言重新表达了一遍。"他把这个感悟念给王振宇听,王振宇难得地认可地点了点头,说这正是他希望陈铭能形成的思维习惯——学一个新框架、新工具的时候,不要只盯着它的API怎么调用,而要多问一句"这个能力对应的是计算机科学里哪个更本质的老问题",这样即便未来框架本身迭代甚至被淘汰,那些更底层的判断力也不会跟着过时。他举了个例子:今天的`RetryPolicy`用的退避算法思想,和很多年前分布式系统里客户端重试RPC调用用的思路是一脉相承的,只是包装成了今天更方便调用的样子;`Send`扇出扇入的模式,本质上也是"分治"思想在Agent编排场景里的一次具体应用。陈铭说他以前学新技术总是习惯性地"这个函数怎么用、那个参数什么意思"地死记硬背,今天算是第一次体会到,如果能往下多刨一层去问"这背后是什么更普遍的原理",记忆反而会更牢,也更能举一反三。
...
陈铭还提到一件让他印象很深的小事:下午做子图练习的时候,他一开始想偷懒,把子图和父图的状态定义写成完全相同的结构,想着"反正字段都一样,省得再写一层映射逻辑"。王振宇看到之后没有立刻说不对,而是问了他一句:"如果三个月之后,父图需要新增一个和子图完全无关的字段,你这个'完全相同'的状态定义,会不会逼着子图也跟着改?"陈铭想了想,说会的,因为共享了同一个类型定义,父图加字段,子图的类型标注上也会多出这个字段,即便子图内部完全不用它。王振宇说这就是"边界不清"带来的隐性耦合——表面上看是省了几行映射代码,实际上是把子图和父图的生命周期焊在了一起,子图失去了独立演进的能力。这件事让陈铭对"子图应该有自己清晰的输入输出边界"这句话有了更具体的体感,不再只是笔记本上的一句话。

王振宇继续追问,如果让陈铭现在给林悦或者祺瑞的技术负责人讲今天学的这几件事,他会怎么用业务语言去讲,而不是甩术语。陈铭想了一下,试着讲了一遍:"持久化,就是不管系统重启多少次,单据的审批进度都不会丢,该等谁批的还是等谁批,不会变成新的单子;人机协同中断,就是系统知道什么时候该'歇一歇'等人做决定,不会傻乎乎地占着资源空转,也不会因为等待太久就把事情搞乱;并行处理,就是原本三件事一件一件排队做,现在可以同时做,总时间明显缩短;重试策略,就是系统遇到网络抖一下这种小毛病,自己能扛住,不用一有风吹草动就找人工救火,但遇到真正解决不了的问题,系统也知道及时喊人,不会瞎重试浪费时间。"王振宇听完说这个讲法已经很接近能讲给客户听的水准了,只是还可以再加一句"这一切发生在客户完全无感的后台,他们只会感觉到审批变快了、单子不会丢了",把技术能力最终落回客户能感知到的结果上,这才是真正打动客户的方式,而不是罗列一堆功能点。
...
临走前,陈铭把笔记本翻回第一页,把王振宇早上那句"没有持久化的中断,只是一个假的中断"和晚上这个新问题并排写在一起,中间画了一个箭头。他说自己隐隐觉得,这两句话之间藏着一条线——今天学的所有东西,都是在让"一个Agent"变得更可靠、更能扛事,但可靠和能扛事,解决的是"怎么把一件事做对"的问题,并不解决"这件事到底该不该由一个大脑来决定"的问题。他把这句感悟写在了笔记本边上,作为留给自己明天验证的一个猜测。
...
**作业八**
今天课堂笔记里提到一个排查案例:陈铭在`outline_human_review`节点里用`try/except Exception`包住了`interrupt()`调用,导致图无法正常挂起。请解释这个现象背后的原理(`interrupt()`的挂起机制与异常处理之间的关系),并说明在实际项目中,如果一个节点确实需要在`interrupt()`调用附近做异常处理(比如捕获别的、和中断无关的异常),应该怎么写才能既保留必要的异常处理能力,又不破坏中断机制的正常工作。

**作业九**
请设计一张状态表,并结合`get_state`与`get_state_history`两个方法,说明如果祺瑞集团的一位业务运营人员想要在系统界面上实现"查询某张报销单当前卡在哪一步、已经过了哪些审批环节、每一步大概停留了多久"这样一个查询页面,后端应该如何组织对LangGraph的调用,才能拿到支撑这个页面所需的全部数据。请具体说明需要读取状态快照里的哪些字段。
...
`interrupt()`之所以能实现"挂起并等待"的效果,底层依赖的是一种类似"通过抛出特殊信号来中断当前执行流"的机制——当代码执行到`interrupt()`这一行时,它会向上层的图执行引擎"报告"当前需要挂起,引擎捕获到这个信号之后,负责把状态持久化、记录挂起位置,并结束本次`invoke`调用,把控制权交还给调用方。如果业务代码在`interrupt()`调用外面包了一层`try/except Exception`(或者任何足够宽泛、能够捕获这类内部信号的异常处理),就相当于业务代码"抢在引擎处理之前"把这个信号拦截并吞掉了,引擎接下来看到的不是"需要挂起"的信号,而是"这个节点执行时抛出了一个被业务代码处理掉的异常",于是引擎不会执行正常的挂起流程,而节点函数本身也因为异常被`except`分支捕获,会继续执行`except`块里的兜底逻辑,导致原本期望的"挂起等待人工输入"的行为完全没有发生。

正确的处理方式,是让`try/except`的捕获范围尽量精确,只捕获自己明确知道、明确要处理的异常类型,而不是用`Exception`这种几乎覆盖一切的宽泛类型。例如:

```python
def approval_node(state):
    try:
        preprocessed = risky_preprocessing(state)
    except (ValueError, KeyError) as exc:
        preprocessed = {"warning": f"预处理数据异常,已使用默认值: {exc}"}
...
在这个写法里,`try/except`只包裹了`interrupt()`调用之前、明确知道可能出问题的那一段预处理逻辑,并且只捕获`ValueError`和`KeyError`这两种具体类型,`interrupt()`调用本身完全暴露在外层,不会被任何异常处理逻辑意外拦截。这样既保留了对已知风险点的兜底能力,又不会破坏引擎正常的中断挂起机制。更进一步的最佳实践是:如果一个节点函数确实很复杂,需要大量异常处理,可以考虑把"可能出错的业务逻辑"和"负责发起中断的那一小段代码"拆成两个独立的节点,用一条普通的边连接起来,这样两部分的异常处理边界在图结构层面就是天然分开的,不需要靠开发者时刻记着"别把try/except写得太宽"这种容易疏漏的约定。

**作业九参考答案**

要支撑"查询某张报销单当前卡在哪一步、已经过了哪些审批环节、每一步大概停留了多久"这样一个查询页面,后端的状态设计与调用方式大致如下:

状态表设计(核心字段):

| 字段名 | 类型 | 用途 |
| --- | --- | --- |
| `bill_id` | str | 报销单唯一编号,同时作为thread_id的组成部分 |
| `current_stage` | str | 当前所处的审批阶段标识,如"finance_review" |
| `stage_history` | list[dict] | 历史上经过的每一个阶段的记录 |
| `pending_role` | str | 当前正在等待谁操作(如"财务复核人员") |

调用方式方面,后端首先需要用`get_state(config)`拿到该报销单对应thread_id的最新状态快照:快照中的`values`字段就是当前完整的状态数据,可以直接读出`current_stage`字段展示"当前卡在哪一步";快照的`next`字段说明了引擎认为接下来会执行哪些节点,如果流程正处于挂起状态,`next`里通常就是那个包含`interrupt()`调用的节点,业务系统可以据此判断"当前是不是在等人工操作";快照的`tasks`字段(如果处于中断状态)里能进一步取出当次`interrupt()`传出的payload,里面通常携带了"当前需要谁来处理、需要处理什么"的具体信息,这部分可以直接映射为页面上"等待XX审批"的文案。

要展示"已经过了哪些审批环节、每一步停留了多久",单靠一次`get_state`是不够的,需要调用`get_state_history(config)`,拿到该thread从创建以来的全部历史检查点列表。这个列表按时间顺序(通常返回时是从最新到最早)记录了每一次状态变化对应的快照,每个快照都带有自己的`created_at`时间戳。业务系统可以遍历这个历史列表,找出`current_stage`字段发生变化的那些检查点(意味着流程从一个阶段推进到了下一个阶段),把相邻两次"阶段变化"检查点的时间戳做差,就能算出"上一个阶段大概停留了多长时间"。把这些信息按时间顺序拼接起来,就能在页面上渲染出一条完整的"审批时间轴":某年某月某日某时,进入班长审批;等待若干小时后,进入项目经理审批;等待若干小时后,进入财务复核……直到当前所处的阶段。这个设计思路本质上完全沿用了上午课堂笔记里提到的"时间旅行"能力,只是把它用在了"面向业务人员的可观测性查询"这个具体场景上,而不是用于纠错回滚。
