...
**关于错误处理与降级策略的讨论。** 这个话题是韩露主动挑起来的,她说:"我们讨论了半天工具该怎么设计输出格式,但工具调用失败之后,Agent 应该怎么反应,好像还没有一个统一的说法。"老王认为这个问题问得很对,现场给出了一个分级方案——把工具调用可能出现的异常分成三类来处理。第一类是"可重试的临时性失败",比如网络超时、下游服务偶发的 5xx 错误,这类失败 Agent 应该在设定的重试次数内(比如最多3次,采用指数退避的等待间隔)自动重试,不需要立刻打扰用户;第二类是"需要降级处理的失败",比如 SQL 查询工具因为生成的 SQL 语句涉及了没有权限访问的表而被拒绝,这种情况重试是没有意义的(重试100次结果都一样),Agent 应该转入降级路径,比如提示用户"该数据涉及更高权限,建议联系数据管理员",而不是死循环重试到超时;第三类是"不可恢复的严重错误",比如数据库连接字符串配置错误、模型网关鉴权失败,这类错误往往意味着系统本身有配置问题,Agent 不应该试图掩盖它,而是要把详细的错误上下文完整记录进日志,同时给用户一个礼貌但明确的"系统暂时无法处理该请求"的提示,方便工程师第一时间从日志里定位。老王补充说,这三类异常在代码实现上应该对应三种不同的异常类型(比如`RetriableToolError`、`DegradableToolError`、`FatalToolError`),而不能笼统地都用一个`Exception`兜底,否则 Agent 层拿到异常之后根本没法区分该怎么应对,只能一刀切地全部报错给用户,体验会很差。这条建议直接影响了后文代码实战部分`core/base.py`里异常体系的设计。

**关于可观测性与日志颗粒度的讨论。** 林悦在讨论快结束时提了一个从客户视角出发的问题:如果客户后续质疑"这个邮件里的数字是怎么来的",项目组能不能在几分钟之内查到完整的证据链?这个问题让老王意识到,可观测性不能只停留在"打个日志"的层面,而要设计成"任务级别可完整回放"——每一次 Supervisor 的任务拆解决策、每一次 Agent 的路由选择、每一次工具的输入输出,都要能够通过一个唯一的任务ID串联起来,形成一条完整的调用链。他现场用白板画了一个简单的例子:如果客户对某封邮件里的一个销售数字提出疑问,工程师应该能够凭这封邮件关联的任务ID,一路查到"这个数字来自哪次 SQL 查询、SQL 语句是什么、查询命中了哪张表的哪些字段、查询发生的时间点",整个过程不应该超过五分钟。这条要求后来被陈铭写进了 PRD 的非功能性需求里,也是`TaskStateStore`在设计时特别保留完整历史记录而不是"只保留最新状态"的原因——如果每次状态更新都覆盖旧值,虽然存储空间更省,但会丢失掉排查问题时最需要的中间轨迹。
...
张凡按这个思路改完代码,自测的时候还是踩了一个坑。他用了 Python 标准库的`datetime.fromisoformat`去解析客户数据里"2026-07-20 14:00"这种带空格分隔的本地时间字符串,结果直接抛出了`ValueError: Invalid isoformat string: '2026-07-20 14:00'`——因为标准的 ISO8601 格式要求日期和时间之间用大写字母`T`分隔,而不是空格,客户数据仓库里导出的却是空格分隔。张凡把这个报错截图发到群里问陈铭,陈铭看了一眼说,这种情况不能指望上游改数据格式(客户的数据仓库不是咱们能随便改的),得在工具内部做一层"宽松解析":先尝试标准 ISO 格式解析,失败了就把空格替换成`T`再试一次,如果还失败,再尝试几种常见的候选格式(比如"%Y-%m-%d %H:%M:%S"和"%Y/%m/%d %H:%M"),全部尝试失败才真正抛出异常,并且异常信息里要把"收到的原始字符串是什么、期望的格式是什么"都写清楚,而不是甩一句`ValueError`让人猜。张凡照着这个思路重写了`_parse_flexible_datetime`辅助函数(完整实现见后文代码实战部分),自测又跑了十几个不同格式的样例,确认都能正确解析。他后来在群里感慨了一句:"以前自己写脚本从来不管这些,反正数据长啥样我都知道,直接写死格式就完事了;这次才发现给别人用的工具,根本没法假设别人传的数据一定规规矩矩。"陈铭回了一句:"这就是工具和脚本的区别,工具是要被别人反复调用的,脚本是自己用一次就扔。"
...
这个小插曲结束后,陈铭多问了张凡一句:"你写的时候是怎么想到用`title`的?"张凡说他是直接照着客户数据仓库表结构里的字段名抄的,那张表里对应的列名就叫`title`,他觉得"顺手保持一致比较省事"。陈铭想了想说,这个思路本身没错,但他自己在设计 Supervisor 展示逻辑的时候,脑子里想的是"邮件标题、日程标题、文档标题"这些不同的"标题"概念未来可能都要在同一个展示层里出现,所以下意识地想加一个前缀做区分,没跟张凡提前对齐。他把这个反思也发进了群里:"这次是我的问题,我预设了一个'未来可能需要区分'的场景,但没有提前告诉你,你当然没法猜到我脑子里在想什么。以后这种可能影响接口设计的想法,哪怕还没最终确定,也应该先说出来讨论一下,不能自己脑子里转一圈就直接在自己的代码里悄悄改了预期。"张凡回复"没事陈老师,这种事儿说清楚就行,下次我们要是再遇到类似的需要提前想清楚数据模型的情况,可以先花十分钟对一下字段表"。这次小小的返工,反而让两人对"接口先行"这件事有了更具体的体感,不只是听老王说教式地强调一遍。
...
苏梦在群里还举了一个她实际测出来的反面案例:她第一版 Prompt 只写了"请根据以下销售数据撰写一封汇报邮件",没有加任何关于"不能编造数字"的约束,结果 LLM 生成的邮件正文里出现了这样一句话——"本季度销售额较去年同期增长约23%,超出预期目标"。她拿着这句话去核对输入素材,发现素材里压根没有"去年同期"这个维度的数据,更没有任何"23%"这个数字,这是 LLM 完全脑补出来的一句"看起来很专业但毫无依据"的表述。苏梦说这个案例给她的触动很大,因为如果不是她刻意去核对,这种"听起来很合理"的编造内容很容易被直接放过去,一旦发到客户那边被追问数据出处,后果会很严重。她把这个案例整理成了一条"反面教材"贴在了团队的共享文档里,提醒大家在验证工具输出的时候,不能只看"格式对不对",更要逐句核对"这句话背后有没有真实依据"。改进后的第二版 Prompt,她加上了一条明确指令:"只允许引用下方素材中明确出现的数据,禁止进行任何形式的同比、环比推算,除非素材中直接给出了对比数值",这条约束加上之后,再测试同样的场景,LLM 就不再"自作聪明"地编造增长率了。
...
韩露还提到一个她在重构过程中发现的小问题:陈铭上周预研代码里,SQL 执行失败时抛出的异常信息是直接把数据库驱动返回的原始报错文本原封不动地暴露了出来,类似"psycopg2.errors.InsufficientPrivilege: permission denied for table sales_detail"这样的内容,如果这条异常信息不加处理地展示给最终用户,不仅不友好,还可能暴露内部表名、字段结构这些不应该让业务用户直接看到的技术细节。韩露的处理方式是加一层"异常翻译层"——把常见的数据库异常类型映射成对用户友好的业务语言,比如权限类异常统一翻译成"您暂时没有权限查询该数据,如有需要请联系数据管理员开通",超时类异常翻译成"查询数据量较大,正在处理中,请稍候",而原始的技术异常信息仍然完整保留在后台日志里,方便工程师排查,只是不会直接吐给用户。陈铭看完这个改动,又想起了老王上午讨论会上说的"三类异常分级处理"的建议,意识到这其实是同一套思路在不同模块里的具体落地——好的错误处理设计,应该是"对内详细、对外友好"这两者兼顾,而不是二选一。
