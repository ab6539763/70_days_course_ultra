...
**演示片段六:跨文档综合性问题(压力测试环节延伸)**

林悦看时间还有余量,主动提议再追加一个稍微复杂一点的场景,试探一下系统在"需要综合多份文档才能回答"这类问题上的表现。她建议陈铭当场输入一个更刁钻的问题:"XJ-3200A如果同时出现主轴过速报警和液压压力异常,应该先处理哪个,会不会互相影响?"

这个问题的特殊之处在于,答案分别分布在《设备报警代码手册》和《设备保养作业指导书》两份不同的文档里,而且需要一定的"先后顺序"判断,不是单纯的检索能力能覆盖的,更接近于"需要综合规则做推理"的问题类型。

系统检索耗时明显比之前几个问题更长,大约5秒左右,给出的回答是:"根据现有资料,主轴过速报警通常提示设备负载或转速异常,液压压力异常(E-15)提示液压系统故障。现有资料中未明确说明两类报警同时出现时的优先处理顺序,建议您优先联系设备科现场判断;若必须自行处理,一般建议先排除对人身安全和设备安全影响更大的报警项。此回答基于现有资料的常规处理经验总结,不能完全替代现场工程师的专业判断。"

陈志强主任对这个回答的态度很直接:"这个回答已经比我预期的好——它没有装作自己什么都懂,而是老老实实说了‘资料中未明确说明’,还给了一个相对负责任的建议。这种问题本来就应该交给现场工程师最终拍板,系统能做到这一步,已经算是尽责了。"

老王事后专门把这段对话记录进了《技术债清单》,标注为"二期需求候选":"跨文档综合推理"如果要真正做扎实,需要引入更复杂的推理链路,这也是团队后续要在Agent方向投入精力的一个具体理由——今天这道题答得"不完美但诚实",恰恰给了整个团队一个非常真实的信号:纯粹的检索式问答,在处理需要综合判断、需要调用规则做推理的问题时,终究会遇到天花板,这个天花板具体在哪里,今天算是被亲手摸到了一次。
...
李师傅这时候又插了一句,带着点不好意思:"我们车间不少老师傅,岁数大了,打字慢,有时候一句话打半天打不出来,这系统能不能直接说话问,不用打字?"

这个问题让会议室里的气氛松弛了一下,林悦笑着回应:"这个问题提得特别实在,谢谢李师傅。目前这一版,我们主要验证的是文字问答的核心能力,语音输入还没有正式接入,但技术上这个功能并不复杂——本质上是先把语音转成文字,再走我们现在这套问答流程,所以只要贵司确认有这个需求,我们后续版本是可以补上语音输入这项功能的。"陈铭在旁边补充了一句技术细节:"目前市面上的语音识别在中文方言、车间嘈杂环境下的识别准确率还需要专门测试和调优,如果后续要上语音输入,我们会针对贵司车间的实际环境(比如设备运转时的背景噪音)做专门测试,确保识别准确率达标之后才正式上线,不会为了赶功能而牺牲体验。"李师傅点了点头,在他那本笔记本上记了一行字,陈铭注意到那行字写的是"下次问问打字的事"。
...
老王临出这个话题前,又多说了一句提醒,提前把评审陈铭时留的一个小尾巴接上:"还有一个小问题,我顺手记一下,不算今天的重构任务,但你们几个都要留意——我看代码里有些地方还在用`print()`打调试信息,比如`RerankService`重构前那段‘正在加载bge-reranker-large模型……’,这在自己本地调试的时候没问题,但代码一旦部署到生产环境,`print()`的输出既不会按级别分类,也不方便集中收集和检索,一旦线上出了问题,你想从几万行日志里找到某一次请求对应的记录,会非常痛苦。我要求你们把项目里所有`print()`调试语句,统一替换成`logging`模块的对应级别调用,这个工作量不大,但我希望你们养成‘从写下第一行代码开始就用日志、不用print’的习惯——今天重构后的`RerankService`已经带头做了示范,你们可以直接参考它的写法。"陈铭低头看了一眼自己写的代码,确实找到了七八处还残留着调试用的`print()`,不好意思地在本子上补记了一条"清理print,统一用logging"。
...
张凡又追问了一个问题,是他在整理`RAGSettings`草稿时想到的:"那如果配置里既有像`chunk_size`这种可以直接写在代码仓库里的参数,又有未来可能涉及的密钥、密码这种敏感信息,是不是也应该放进同一个配置类里统一管理?"

老王摇了摇头,态度很明确:"不是同一类东西,不能混在一起管理。像`chunk_size`、`top_k`这类参数,泄露出去没有任何安全风险,写进代码仓库、提交到Git历史里都无所谓;但API密钥、数据库密码这类敏感信息,一旦被提交进Git仓库,即使后来删除,依然可能留在提交历史里被人找到,这是完全不同性质的风险。Pydantic Settings这类工具确实可以同时管理这两类配置,但我要求你们做一个团队约定:所有敏感信息必须通过环境变量或者专门的密钥管理服务读取,绝对不能出现在`.env`文件之外任何一处会被提交进版本库的文件里;而像`chunk_size`这种业务参数,写进配置文件、提交进仓库,反而是好事,因为这样团队所有人都能看到当前生效的参数是什么,不需要去挨个问‘谁改过这个值’。"他顺手在白板上补了一条团队规范:"配置分两类——‘可以public的业务参数’和‘绝对不能public的敏感凭证’,永远不要把它们塞进同一个文件、用同一套管理方式。"这条规范后来被写进了《苍穹项目工程规范手册》第三章,成为团队后续所有项目配置管理的基本准则。
...
陈铭在完成`RerankService`抽取之后,又多做了一件老王没有明确要求、但他自己觉得应该做的事——为这个新服务补了一份简单的单元测试,覆盖了"候选列表为空""全部候选低于阈值触发兜底""正常重排序返回结果"三种典型场景。他把这个想法在群里说了一下,老王难得地回了一句"这个习惯保持下去":"很多人重构完代码,只满足于‘我肉眼看过一遍,应该没问题’,但‘应该没问题’和‘有测试证明没问题’之间,差的是能不能在未来任何一次改动之后,快速知道自己是不是把什么东西改坏了。你今天补的这三个测试用例,以后不管是你自己改,还是苏梦、韩露、张凡谁接手这段代码去改,只要跑一下测试,几秒钟就知道有没有破坏原有行为,这比‘我记得这里逻辑是这样的’靠谱得多。"这句话后来也被记进了《苍穹项目工程规范手册》,作为"重构必须配套补充或更新单元测试"这条规范的由来案例。
